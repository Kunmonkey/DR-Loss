...
def eval_map(results,
    ...
    """
    results = [torch.sigmoid(i).cpu().numpy() for i in results]
    ...
    # print(results)
    int_results = np.rint(results)
    # print(np.where(int_results<0))
    # Rec = recall_score(gt_labels, int_results, average=None)
    # Pre = precision_score(gt_labels, int_results, average=None)
    APs = average_precision_score(gt_labels, results, average=None)
    mAP = APs.mean()
    # mAP = average_precision_score(gt_labels, results, 'samples')
    if print_summary:
        print_map_summary(mAP, APs, dataset)
    return mAP, APs #, Pre, Rec
