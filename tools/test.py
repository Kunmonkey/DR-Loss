...
def single_gpu_test(model, data_loader, show=False):
    ...
    for i, data in enumerate(data_loader):
        ...
        # labels.append(gt_labels.cpu().numpy())
        # file_name.append(data['file_name'].cpu())
        if show:
        ...
        for _ in range(batch_size):
            prog_bar.update()
    # res = dict(pred=results,labels=labels,file_name=file_name)
    # mmcv.dump(res,'F:/论文代码/样本不平衡/DistributionBalancedLoss-master/work_dirs/LT_voc_resnet50_pfc_DB/pred.pkl')
    return results
    ...
    if args.from_file:
        ...
        for d, dataset in enumerate(dataset_list):
            ...
            for i in range(len(dataset)):
                ...
                img_infos = dataset.img_infos[i]
                gt_labels.append(gt_ann['labels'])
                file_name.append(img_infos['filename'])
            ...
            if rank == 0:
                savedata[d].update(gt_labels=gt_labels,outputs=np.vstack(outputs),file_name=file_name)
        if rank == 0:
